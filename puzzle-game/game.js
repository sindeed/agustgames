...
const LASER_BOSS_SPIN_MS = 2000;      // ett helt laservarv
...
function update(now) {
  ...
  if (secondPlayer &&
      player.col === secondPlayer.col &&
      player.row === secondPlayer.row &&
      !(tileAt(player.col, player.row) === "goal" && checkDualGoalWin())) {
    hit(secondPlayer);
    if (state !== "playing") return;
  }
...
function drawMeteor(meteor, now) {
  ...
  for (let dr = -radius; dr <= radius; dr++) {
    for (let dc = -radius; dc <= radius; dc++) {
      ...
      roundRect(tx + 4, ty + 4, TS - 8, TS - 8, 7);
      ctx.fill();
      ctx.strokeStyle = landed ? "rgba(255,170,80,0.35)" : `rgba(255,160,50,${0.18 + warnP * 0.45})`;
      ctx.lineWidth = 2;
      roundRect(tx + 4, ty + 4, TS - 8, TS - 8, 7);
      ctx.stroke();
    }
